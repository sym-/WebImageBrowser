...
- (void)loadView
{
    ...
    isWhite = [[UIApplication sharedApplication] statusBarStyle] == UIStatusBarStyleLightContent?YES:NO;
...
- (void)createToolbar
{
    ...
    _saveImageBtn.frame = CGRectMake(self.view.width - 40, self.view.height-40, btnWidth, btnWidth);
    ...
    _saveImageBtn.backgroundColor = [UIColor colorWithHue:0 saturation:0 brightness:0 alpha:0.6];
    _saveImageBtn.layer.cornerRadius = 4.f;
...
- (void)saveImage
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (photo.save == YES) {
            [SVProgressHUD showSuccessWithStatus:@"您已保存"];
            return;
        }
...
- (void)photoViewSingleTap:(MJPhotoView *)photoView
{
    ...
    isWhite == YES? [[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleLightContent animated:YES]: [[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleDefault animated:YES];
...
- (void)loadImageNearIndex:(int)index
{
    if (index > 0) {
        ...
        [photo imageWithProgressBlock:nil competionBlock:nil];
    }
